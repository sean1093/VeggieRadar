...
const DetailDrawer: React.FC<DetailDrawerProps> = ({ isOpen, onClose, item, allProduceItems, watched = false, onToggleWatch }) => {
  ...
  // Alternative suggestions: strictly cheaper items in the same category.
  const suggestions = allProduceItems
    .filter((p) => p.category === item.category && p.code !== item.code && p.catty_price < item.catty_price)
    .sort((a, b) => a.catty_price - b.catty_price)
    .slice(0, 3);
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-stone">約 {item.avg_price.toFixed(1)} 元/公斤</p>
            </div>
            <div>
              <p className="text-xs text-stone">與昨日相比</p>
                ...
                {suggestions.map((s) => (
                  <div key={`${s.code}-${s.name}`} className="flex items-center justify-between rounded-xl bg-sage-soft/50 px-4 py-3">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-sage tabular-nums">{s.catty_price.toFixed(1)} 元/台斤</p>
                      <p className="text-xs text-stone tabular-nums">省 {(item.catty_price - s.catty_price).toFixed(1)} 元/台斤</p>
                    </div>
                  </div>
                ))}
              </div>
