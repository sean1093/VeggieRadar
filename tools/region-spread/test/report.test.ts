...
function render(rows: MoaRow[]): string {
  ...
  return renderReport(META, marketSightings([{ def: CABBAGE, rows, days }]), [cropStats(CABBAGE, days)]);
}
...
describe('renderReport', () => {
  ...
  it('will not let a --root run’s green mapping check stand for the whole roster', () => {
    const md = renderReport({ ...META, partial: true },
      marketSightings([{ def: CABBAGE, rows: NORTH_AND_CENTRAL, days: dailySplit(NORTH_AND_CENTRAL, CABBAGE) }]),
